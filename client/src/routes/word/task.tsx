...
export const Route = createFileRoute('/word/task')({
  loader: ({ context }) => loadSessionForPhases(context, ['word-items']),
  component: WordTaskPage,
});

function WordTaskPage() {
  ...
  const itemId = current?.phase === 'word-items' ? current.item.itemId : null;
  ...
  const handleSubmit = async () => {
    if (!selectedValue || !current || current.phase !== 'word-items') return;
  ...
  return (
    <FlowRouteFrame session={session} isPending={isPending} isError={isError} onRetry={() => void refreshAndNavigate()}>
      {current?.phase === 'word-items' ? (
        <TaskItem
          title={t('wordTaskPrompt')}
          description={t('wordTaskDescription')}
          candidates={current.item.candidateWords.map((word): TaskCandidate => ({ value: word, label: word }))}
          value={selectedValue}
